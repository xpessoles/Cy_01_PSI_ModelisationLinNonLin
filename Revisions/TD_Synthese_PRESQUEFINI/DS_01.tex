...
\def\classe{PSI$\star$}
\def\xxnumpartie{{Num Partie}}%\textsf{\textsf{Cy. 4, 6 \& 7}}}
\def\xxpartie{Partie}


\def\xxnumchapitre{21 septembre 2020 \vspace{.2cm}}
\def\xxchapitre{\hspace{.12cm} Chapitre}
...
\def\xxonglet{\textsf{TD **}}%\textsf{\textsf{Cy. 4, 6 \& 7}}}

\def\xxactivite{\textsf{TD ***}}
...
\subparagraph{}\textit{ Exprimer la fonction de transfert de l’ensemble \{bateau support + ROV + PHC\}, $G(p) = \dfrac{Y_{\text{ROV}}(p)}{Y_{\text{vague}}(p)}$ en fonction de $H(p)$ et $B(p)$. Tracer en rouge l'allure du gain du diagramme de Bode de $G(p)$.}
\ifprof
\begin{corrige}
%\textbf{Q17}  
