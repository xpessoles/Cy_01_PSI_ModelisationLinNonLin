...
%Moment d’inertie du réducteur ramené à l’arbre moteur&	$J_{réd}$ & $\SI{60.10^{-4}}{kg.m^2}$ \\ \hline
%Moment d’inertie de la poulie motrice autour de son axe&	$J_{PM}	&$ \SI{38.10^{-4}}{kg.m^2}$ \\ \hline
%Moment d’inertie de la poulie réceptrice autour de son axe&	$J_{PR}$ & $\SI{38.10^(-4)}{kg.m^2}$ \\ \hline
...
\end{center}
\end{document}
À partir des grandeurs définies déterminer l’expression littérale de l’inertie équivalente J_eq de l’ensemble Σ={moteur+réducteur+poulies+chariot} ramenée sur l’arbre moteur. Cette inertie équivalente est définie par E_c (Σ)=1/2 J_eq ω_m^2.
...
À partir des équations du moteur à courant continu, réaliser le schéma bloc du moteur à courant continu.
En considérant que C_R (p)=0, déterminer la fonction de transfert H_M (p)=(Ω_m (p))/(U(p)) sous sa forme canonique.
Montrer que la fonction de transfert H_M (p) peut se mettre sous la forme H_M (p)=K_C/(K_C K_e+RJ_eq p+LJ_eq p^2 ). Justifier la réponse. Pour cette question, la valeur numérique de J_eq considérée sera J_eq=7⋅10^(-3)  kg m^2 indépendamment du résultat numérique calculé précédemment.
Montrer qu’avec l’expression, H_M (p) peut s’écrire sous la forme H_M (p)=K_M/(1+T_E p)(1+T_M p)  avec T_E<T_M..
