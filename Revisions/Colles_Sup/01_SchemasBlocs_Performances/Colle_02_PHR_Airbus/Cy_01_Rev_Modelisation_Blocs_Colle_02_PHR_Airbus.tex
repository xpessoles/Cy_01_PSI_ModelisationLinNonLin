...
\subparagraph{}
\textit{Calculer les fonctions de transfert (FT) suivantes et donner à chaque fois la classe et l'ordre : 
\begin{itemize}
\item FT du vérin non asservi : $A_1(p)=\dfrac{X(p)}{Q(p)}$;
\item FT de la chaîne directe : $C(p)=\dfrac{X(p)}{\varepsilon(p)}$;
\item FT boucle ouverte du système : $G(p)=\dfrac{U_s(p)}{\varepsilon(p)}$;
\item FT en boucle fermée du système : $H(p)=\dfrac{X(p)}{U_c(p)}$.
\end{itemize}}
\ifprof
\begin{corrige}
